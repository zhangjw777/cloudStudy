		...
		<div class="fx headerInfo">
			<div class="fx-1 marg-lt-20">
				<div class="fx" v-show="route.meta.title != '首页'">
					<!-- route.matched[0] - 当前路由的匹配记录数组中的第一项（通常是最顶层路由） route.matched[0].path - 该路由记录的路径
					route.matched[0].meta.title - 路由元信息中定义的标题
          {{ route.matched[0].meta.title }} - 使用Vue的插值语法显示路由标题 
          route.meta.fmeta - 路由元信息中定义的父级路由元信息
          -->
					<span class="textDefault1" @click="() => $router.push({ path: route.matched[0].path })">{{ route.matched[0].meta.title }}</span>
					<span class="line"> / </span>
					<span v-if="route.meta && route.meta.fmeta" class="textDefault1" @click="() => $router.push({ path: route.meta.fmeta.path })">{{
						route.meta.fmeta.title
					}}</span>
					<span v-if="route.meta && route.meta.fmeta" class="line"> / </span>
					<span class="textDefault1 ft-cl-des" @click="() => $router.push({ path: route.matched[1].path })">{{
						route.matched[1].meta.title
					}}</span>
				</div>
				<!-- 路由标题等于首页，则显示工作台 -->
				<div class="fx" v-show="route.meta.title == '首页'">
					<span class="textDefault1" @click="() => $router.push('/')"> 工作台 </span>
				</div>
			</div>
			<div class="fx-al-ct">
